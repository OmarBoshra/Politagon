...
class GameDialogs {
  static void showPointStatus(BuildContext context, GameState state) {
    ...
    final totalDecided = ScoreCalculator.calculateTotalPlayerScore(state);
    final decidedPercent = (totalDecided / totalPoints).clamp(0.0, 1.0);
    final thresholdPercent = state.decidedThresholdPercentage;
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _buildSummaryRow('TOTAL ELECTORATE', '$totalPoints', Colors.white),
                _buildSummaryRow('DECIDED VOTERS', '$totalDecided (${(decidedPercent * 100).toStringAsFixed(1)}%)', const Color(0xFFC5A059)),
                _buildSummaryRow('UNLOCK THRESHOLD', '${(totalPoints * thresholdPercent).toInt()} (${(thresholdPercent * 100).toInt()}%)', Colors.white70),
                  ...
                  children: [
                        ...
                        FractionallySizedBox(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(4),
                              boxShadow: [
                                BoxShadow(color: const Color(0xFFC5A059).withOpacity(0.3), blurRadius: 4)
                              ],
                            ),
                          ),
                        ),
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text('${(decidedPercent * 100).toInt()}%', style: const TextStyle(color: Color(0xFFC5A059), fontSize: 9, fontWeight: FontWeight.bold)),
                          Text('GOAL: ${(thresholdPercent * 100).toInt()}%', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 9)),
                        ],
                      ),
                    ),
                  ],
                ...
                // UNDECIDED SECTION
                _buildSectionHeader('UNDECIDED CONSTITUENCY ($totalUndecided)', Colors.grey, Icons.help_outline),
